...
def start_timer():
    ...
    if reps % 8 == 0:
        count_down(long_break_sec) # set time in short break
        label.config(title="Break", fg=RED)
    elif reps % 2 == 0:
        count_down(short_break_sec) # set time in long break
        label.config(title="Break", fg=PINK)
    else:
        count_down(work_sec)  # set time in work
